...
SELECT p.plantId, a.actionId, t.dateTimeStamp
FROM Plant p, ActionTbl a, Tended t
WHERE t.actionId = a.actionId AND t.plantId = p.plantId AND a.watered = 1


--spGetPlantsTendedByPlantType --Get a list 
SELECT p.*
FROM Plant p, PlantType pt, Tended t
WHERE p.plantTypeId = pt.plantTypeId AND t.tendedId = p.tendedId
